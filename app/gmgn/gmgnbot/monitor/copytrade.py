...
class CopyTradeService:
    ...
    async def add(self,
                  copytrade: CopyTrade,
                  *,
                  session: AsyncSession = NEW_ASYNC_SESSION) -> None:
    ...
    async def addall(self,
        ...
        assert model[0].id is not None, "model.id is None"
        # 写入 redis
        await self.monitor_event_producer.resume_many_monitor(
            model[1].id,  # 使用数据库生成的 ID
            ','.join([db_monitor.target_wallet for db_monitor in model]),
            model[1].chat_id,
        )
    ...
    async def update(self,
        ...
        stmt = select(CopyTradeModel).where(
            CopyTradeModel.id == copytrade.pk).limit(1)
    ...
    async def getpk(self,
                    chat_id,
                    session: AsyncSession = NEW_ASYNC_SESSION) -> int:
    ...
    async def get_by_id(
            ...
            pk: int,
            *,
            session: AsyncSession = NEW_ASYNC_SESSION) -> CopyTrade:
